...
(S) Got conflict with my coworkers when I designed virtual nodes' heat detection and mitigation. (T) I proposed a new library/package for detecting heat and mitigate it. But my coworker suggested to integrate this with our current design since if I create new package it increase dependencies to other packages for our project. (A) So I arranged one-to-one call with him and explained the advantages that we may get if we make this one as library. He explained library needs to be maintained separtely but if we integrate it with current design, we don't need to think about the upgrade option. (R) We concluded to make a new library for this one with some suggestions from him.
...
(S) One of my team members was struggling on the polling task. (T) The task was to poll metrics for some cache nodes from Host Manager service. (A) I stepped into the task and explained what should we need (service principles, white listing of accounts etc) to get data for oversubscribing. Also I provided some documents which explains why this is needed (security, quota etc) and how to achive them. (R) After some guidelines, I saw he could meet the deadline and was interested to take new challanges willingly. I also learned many things and became clear about some concepts.
...
(S) In my current team I created an alarm to detect if any DMSHost monitors more/less vitual nodes than prediction (anomaly detection). (T) I tested this one with few virtual nodes and was working fine. But when it went to the production, DMSHost is created according to load. So it triggered lots of alarms frequently. It caused problem to the person who was oncall.(A) I had to fix that with CPU percentage usage of DMSHost. Then I wrote down that mistakes into my list so that I never repeat them again. (R) Now before submitting any changes of my code I recheck if source code and planned (Documents) are same or not. Also check my mistake list so that I never repeat those mistakes again.
...
(S) I noticed that there are lots of configurations that need to be updated after project deployment. (T) So I proposed a solution so that we can change the configurations without project redeployment. Before joining to the team I was looking the services that AWS provides currently. I found AWS AppConfig which will server our case. (A) I read the documentation about it and pros and cons about AppConfig. I designed, implemented and integrated AppConfig that works not only for changing configurations but also custom configurations for users! (R) It reduces our time to think what will be optimal configuration values since we can change the configurations instantly without affecting our project.
...
(S) It happened at the very beginning of my joining in this company. (T) Just after my onboarding I saw my teammates were raising commits to review. I noticed every commits were been reviewed by my teammates so I didn't review. After 2 weeks I got feedback from my manager that since I am L2 engineer so I had to review all the commits. (A) I agreed with my manager about it and from that time I started reviewing all commits that are going from our team. (R) This feedback not only helped me to be good reviewer but also grasp code flow of my team quickly. I appriciated manager's feedback.
...
(S) When I joined in my team I found that onboarding for new joiner wasn't properly documented for my team. (A) I started documenting them in sequencial order so anyone just need to follow this to be onboarded. Whenever I shared this with my manager she appreciated my approach and suggested me to document them to the onboarding plan which be used for new joiners. (R) Now that task is proper documented and any new joiner just need to follow in my team.
...
(S) It happened when I started working on load testing of my project. (T) I had to add an extra optional parameter with the caller service so that we know that request is for testing. (A) We had meeting about this one with that service and all agreed. But whenever I slacked him to add he wan't responding. I sent email keeping our managers at cc therefore I wan't getting any response. So in a common day I went to his desk and collaborately added that parameter to his service. (R) After adding that one I could know for which accound the call was being made and procceded the task accordingly.
